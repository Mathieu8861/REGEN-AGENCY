...
- **03/07/2026 (grosse session compta + perf ads)** :
  ...
  - **Attribution salaires par personne — source de vérité** : exports nominatifs Swan dans `comptabilité/salaire Mathieu.csv` + `comptabilité/salairejaemeson.csv` (2024→2026). Script (scratchpad, re-créable : match `tx_date`+`label`+`amount` via service key `.env.local`, User-Agent `regen-agency-cli/1.0`) → 52 lignes attribuées + verrouillées `manual_override=true`. URSSAF splitté par compte : `...5138` = Mathieu (777+644/mois), `...7874` = Jaemeson (681/mois). Autres décisions : « Demande NNN » = régularisation URSSAF ; commissions janv = reversées Océane (sous-traitance) ; PrestaShop 76,80 = achat pour client ; « FACTURE N° 00000361 » 120 € = avocat Richard (retouche contrats, Juridique). **À l'import des relevés 2024 + juin 2026 : relancer le script d'attribution** (32 lignes en attente).
...
## Prochaines étapes
- **Compta** : identifier/bloquer le mystère 19 €/mois via l'app Swan · importer relevés 2024 + juin 2026 puis relancer l'attribution salaires · déposer les captures « parcours facture » dans `assets/images/compta/` · OVH à ~15 €/mois de moyenne ? · vérifier 1ère facture Pennylane (29 € vs 14 €)
