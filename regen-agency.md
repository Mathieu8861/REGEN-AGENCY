...
- **03/07/2026 (grosse session compta + perf ads)** :
  ...
  - **Budget & Planning ouvert aux clients en écriture** (migration **023**, demande Mathieu : le client remplit lui-même son planning) : budget **prévisionnel** de ses marques (les cellules `is_actual` sync Google/Meta restent intouchables même via l'API), promos en création/édition/suppression, upload/suppression de visuels limité au dossier `{client_id}/` du bucket. UI : tout utilisateur connecté édite, la RLS est le garde-fou. Vue groupe consolidée (sélecteur principal) = lecture seule. **Testé de bout en bout avec un user client jetable : 11/11** (écriture OK, triche is_actual bloquée, isolation entre clients OK, storage OK). Note : un des 3 comptes clients E&CO n'a que 5 marques sur 6 (manque Emile Henry) : à confirmer si voulu.
  - Commits `67e8324` → `95b1501` poussés (auto-deploy Vercel).

- **10/07/2026 (chantier services site public, base : audit 122 prestations réelles vs 46 affichées)** :
  - **Lot 1** : « SEA / SMA » → **« Publicité en ligne »** partout ; suppression du jamais-vendu (Formations SEA + ancre morte, Pinterest/Snapchat, hotmail corrigé) ; carte « Avis Google & E-réputation » remplace « Formations SEA » (index + services).
  - **Lot 2 refondu après retour Mathieu** (2 sections d'offres jumelles = interdit ; grille 3+1 = moche) : section **Packs acquisition** = bandeau sombre asymétrique Full Acquisition 1 450 € (ruban ambré, chips, décor) + 2 packs compacts (Visibilité Locale 360° 500 €, Croissance 900 €) : esthétique volontairement distincte des formules. **Page dédiée `e-reputation.html`** (SEO head complet + JSON-LD Service, sitemap à jour) : hero sombre, bandeau stats ambré, 4 tuiles 2×2 colorées, section langues, process 3 étapes, CTA pack. Sur ads-sea : simple bandeau teaser vers la page.
  - **Système de slots d'illustrations 3D Firefly** : `assets/images/erep-hero-3d.webp` + `erep-avis-3d.webp` attendus (onerror = la page reste propre sans). Prompts fournis à Mathieu : même style que equipe-3d/accueil.
  - **Reste : lots 3-4** : carte « Création de site web » sur index/services (best-seller invisible !), rétrograder carte CRO, sections maintenance/hébergement + audit offert + migration presta sur site-marketing.html.

- **10-15/07/2026 (illustrations 3D + header verre + hero vidéo accueil)** :
  - **Illustrations e-réputation intégrées** : `erep-hero-3d.webp` (boutique 5 étoiles, hero) + `erep-avis-3d.webp` (avis multilingues, section langues : source `erep-avis-3d.png` gitignorée). Hero e-réputation **nettoyé de toute déco** (halo/cercle/dots/chip) sur retour Mathieu : « mettre des trucs pour mettre des trucs = effet inverse » : règle gravée en mémoire utilisateur.
  - **Entrée menu manquante** : « Avis Google & E-réputation » ajoutée au dropdown Services + footer des 11 pages (la page n'était accessible que par cartes).
  - **DA illustrations tranchée : pâte à modeler chaleureuse assumée** (vs glossy tech, mieux adapté aux clients TPE/PME). Étalon de style = `erep-avis-3d.png` en référence Firefly. Leçons prompts : jamais de hex ni de mots descriptifs entre guillemets (rendus tels quels), compter explicitement (étoiles/feuilles), « faceless NO facial features » sinon visage, « adult proportions NOT chibi », pour la vidéo bannir « clay » (déclenche stop-motion plasticine) et toujours « static camera + cible de taille finie ».
  - **Hero accueil refondu** : vidéo 3D `assets/videos/video_banner_accueil.mp4` (personnage arrosant des barres de croissance qui bourgeonnent + étoile dorée : concept « courbe qui bourgeonne » validé après ~10h d'itérations Firefly abandonnant plante/format bannière) en **pleine largeur, boucle 5s** (1re/dernière frame quasi identiques), calée sur son ratio 16:9 jamais rognée, bord gauche fondu dans un dégradé assorti ; texte HTML superposé à gauche + **voile latéral pleine hauteur** (pas de halo radial = « pas naturel ») ; dégradé du titre en verts francs (jamais noir). Mobile : texte au-dessus, vidéo dessous. Ancienne `hero-illustration.mp4` (2D flat) plus référencée, fichier à supprimer un jour.
  - **Header « liquid glass » (réf Apple WWDC25)** : fond blanc 8 % + blur 28px saturate 170 % + liseré et reflet inset. Pages à hero sombre (`e-reputation`, `qui-sommes-nous`) : classe **`header--on-dark`** = verre teinté sombre + texte blanc + logo inversé (filter) tant que non `.scrolled` ; scrollé = verre blanc 60 % partout. Bouton Espace Utilisateur en vert foncé dans le header (contraste).
  - Commits `09fb43e` → `b8e6ade`. ⚠️ Le panneau preview Claude a des captures/scroll-events morts par intermittence : vérifier via DOM/JS, Mathieu juge le visuel sur prod.

- **15-16/07/2026 (accueil finalise + page contact v2, session jusqu'a 1h du matin)** :
  - **Hero accueil peaufine** : reequilibrage lateral (conteneur texte elargi `clamp(1200px,86vw,1460px)`, video decollee du bord droit + fondu des 2 bords : la scene s'arrete a 89% de la video, mesure au pixel) ; voile lateral pleine hauteur pour la lisibilite (PAS de halo radial = juge « pas naturel ») ; degrade du titre en verts francs sans fondu au noir ; texte rework : badge « Bienvenue chez Regen », titre raccourci gardant « generer de la croissance » (generer = echo Regen, VOULU par Mathieu), sous-titre « Trois leviers, un seul objectif : faire pousser vos ventes », **2 CTA ajoutes** (aucun avant !).
  - **Header final** : pill compacte centree (width max-content : fini le vide au milieu), flou reduit 16px + givre 22%, bouton Espace Utilisateur en pastille de verre avec icone user en mask CSS (suit la couleur du texte), variante on-dark OK.
  - **Reste de l'accueil realigne** : carte expertise « Creation de sites web » (icone ambre, lien site-marketing) remplace « Optimisation e-commerce » ; CTA final « Pret a faire pousser votre entreprise ? » → contact.html ; **fausse section blog supprimee** (3 articles factices, liens morts vers blog/ inexistant) ; carrousel realisations passe a 4 cartes visibles (276px) ; vignettes collaboration 01/02/03 sur fond vert sapin texte blanc. Les illustrations services/collab existantes sont DEJA dans la DA 3D : gardees.
  - **Page contact refaite de zero** (inspiration floapp.fr/demo, realise par Regen) : hero sombre + 4 atouts verre (24h/audit offert/sur-mesure/sans engagement), carte humaine Mathieu (photo, citation, coordonnees, sticky), **formulaire 2 etapes** (besoin : chips services + budget + message → coordonnees : prenom/nom/email/tel/entreprise/site) avec barre de progression, validation par etape, envoi mailto pre-rempli structure (comme FLO) + message de confirmation ; « Comment ca se passe » 3 etapes (reutilise .erep-steps) ; **illustration boite aux lettres 3D** (`contact-3d.webp`, source `page_contact.png` gitignoree) a cheval section/footer avec flottement CSS (cachee <760px). SEO head complet ajoute.
  - **En attente Mathieu** : (1) quelle adresse email officielle ? le site utilise `contact@regen-agency.com` (mailto formulaire + CTA) mais la memoire agence dit `contact.regenagency@gmail.com` : verifier que l'adresse domaine existe ; (2) animation de l'illustration contact (image→video Firefly, prompt fourni) ; (3) branchement formulaire → CRM Supabase (prospects) avant migration domaine.
