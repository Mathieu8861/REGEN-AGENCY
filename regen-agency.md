...
- **03/07/2026 (grosse session compta + perf ads)** :
  ...
  - **Page séparée** : le module relevés vit dans **`admin/releves.html`** (entrée sidebar « Relevés bancaires ») ; `admin/comptabilite.html` = dépôt documents + abonnements + récap dépenses uniquement (plus de Supabase).
